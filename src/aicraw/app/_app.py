...
# Console static dir: env, or aicraw package data (console), or cwd.
_CONSOLE_STATIC_ENV = "AICRAW_CONSOLE_STATIC_DIR"


def _resolve_console_static_dir() -> str:
    if os.environ.get(_CONSOLE_STATIC_ENV):
        return os.environ[_CONSOLE_STATIC_ENV]
    # Shipped dist lives in aicraw package as static data (not a Python pkg).
...
if os.path.isdir(_CONSOLE_STATIC_DIR):
    ...
    def _console_icon():
        f = _console_path / "aicraw-symbol.svg"
        if f.is_file():
